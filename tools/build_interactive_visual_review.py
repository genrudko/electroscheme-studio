...
The page is a geometry/UX prototype. It regenerates busbar previews from the
parametric model and places every preview into one SVG coordinate system:

    grid == busbar == labels == guides == drag coordinates

It also prototypes copy/paste relative to a picked reference point:

    select text -> Copy by reference point -> click base point -> click paste point(s)

This is intentionally model-free for now: pasted labels are visual clones inside
the review page. The editor contract is documented separately.
"""
...
<svg
  ...
  data-reference-copy-paste="true"
...
{preview.svg_fragment}
  ...
  <p class="drag-status">Drag text. Copy by reference point: select text → Copy by reference point → click base point → click paste point(s).</p>
</article>"""
...
<head>
  ...
  <style>
    ...
    [data-role="bus-label"], [data-role="bay-label"], [data-role="free-text-box"] { cursor: grab; user-select: none; }
    [data-role="bus-label"].selected, [data-role="bay-label"].selected, [data-role="free-text-box"].selected {
      paint-order: stroke;
      stroke: rgba(37, 99, 235, 0.35);
      stroke-width: 3;
    }
    [data-role="free-text-box"] { fill: #111111; }
    ...
    .toolbar .group-title { margin: 10px 0 6px; color: #93c5fd; font-size: 12px; font-weight: 800; text-transform: uppercase; letter-spacing: .04em; }
    .toolbar button {
      margin: 3px;
      padding: 6px 9px;
      border: 0;
    ...
    .toolbar .mode { padding: 7px 9px; border-radius: 10px; background: rgba(59, 130, 246, .18); color: #dbeafe; line-height: 1.35; }
  </style>
</head>
<body data-copy-paste-mode="reference_point_copy_paste_visual_prototype">
  <!-- copy_paste_mode: reference_point_copy_paste_visual_prototype -->
  <header class="page">
    <h1>Clean interactive busbar review</h1>
    <p>One SVG coordinate system per card. Text can be dragged, rotated, copied normally or copied/pasted relative to a picked point.</p>
  </header>
  <main>
    __CARDS__
  </main>
  <aside class="toolbar">
    ...
    <p class="group-title">Reference copy/paste</p>
    <div>
      <button id="copyCenterButton">Copy from text center</button>
      <button id="copyByReferenceButton">Copy by reference point</button>
      <button id="pasteByPointButton">Paste by point</button>
      <button id="cancelModeButton" class="secondary">Cancel</button>
      <button id="clearCopiesButton" class="danger">Clear copies</button>
    </div>
    <p id="modeStatus" class="mode">Mode: select text or drag labels. Ctrl+C copies from center, Ctrl+V starts paste-by-point, Esc cancels.</p>
  </aside>
  <script>
    let selectedText = null;
    let clipboard = null;
    let currentMode = 'normal'; // normal | pickBase | pasteByPoint
    let generatedCopyCounter = 0;
    ...
    function setMode(mode, message) {
      currentMode = mode;
      document.getElementById('modeStatus').textContent = message;
    }
    ...
    function selectTarget(target) {
      ...
      if (target) {
        ...
        const id = target.getAttribute('data-bay-slot-id') || target.getAttribute('data-copy-id') || role;
    ...
    function ensureMarkerLayer(svg) {
      let layer = svg.querySelector('[data-role="reference-copy-markers"]');
      if (!layer) {
        layer = document.createElementNS('http://www.w3.org/2000/svg', 'g');
        layer.setAttribute('data-role', 'reference-copy-markers');
        layer.setAttribute('pointer-events', 'none');
        svg.appendChild(layer);
      }
      return layer;
    ...
    function drawReferenceMarker(svg, point, kind) {
      const layer = ensureMarkerLayer(svg);
      const color = kind === 'base' ? '#f59e0b' : '#22c55e';
      const size = 5;
      const group = document.createElementNS('http://www.w3.org/2000/svg', 'g');
      group.setAttribute('data-marker-kind', kind);
      addLine(group, { x1: point.x - size, y1: point.y, x2: point.x + size, y2: point.y, stroke: color, 'stroke-width': '1.5' });
      addLine(group, { x1: point.x, y1: point.y - size, x2: point.x, y2: point.y + size, stroke: color, 'stroke-width': '1.5' });
    ...
    function copyStyleAttributes(source, clone) {
      const attrs = [
        'font-family',
        'font-size',
        'font-weight',
    ...
    function buildClipboardFromSelected(basePoint) {
      if (!selectedText) {
        setMode('normal', 'Mode: select a text label first.');
        return false;
      }

      const anchor = textAnchor(selectedText);
      const center = bboxCenter(selectedText);
      const rotation = Number(selectedText.getAttribute('data-rotation') || '0') || 0;
      const text = selectedText.textContent || '';

      clipboard = {
        kind: 'text',
        text,
        role: selectedText.getAttribute('data-role') || 'free-text-box',
        sourceBaySlotId: selectedText.getAttribute('data-bay-slot-id') || '',
        anchor,
        center,
        basePoint,
        rotation,
        attributes: {
          fontFamily: selectedText.getAttribute('font-family') || 'Arial, sans-serif',
          fontSize: selectedText.getAttribute('font-size') || '16',
          textAnchor: selectedText.getAttribute('text-anchor') || 'start',
          dominantBaseline: selectedText.getAttribute('dominant-baseline') || 'middle',
          fill: selectedText.getAttribute('fill') || 'var(--label-color, #111111)',
        },
      };

      drawReferenceMarker(selectedText.ownerSVGElement, basePoint, 'base');
      return true;
    }

    function copySelectedFromCenter() {
      if (!selectedText) {
        setMode('normal', 'Mode: select text first, then copy.');
        return;
      }
      const center = bboxCenter(selectedText);
      if (buildClipboardFromSelected(center)) {
        setMode('pasteByPoint', 'Mode: copied from text center. Click target paste point. Ctrl+V keeps paste mode active.');
      }
    }

    function beginCopyByReferencePoint() {
      if (!selectedText) {
        setMode('normal', 'Mode: select text first, then choose Copy by reference point.');
        return;
      }
      setMode('pickBase', 'Mode: click the reference/base point. The copied text will keep this offset on paste.');
    }

    function beginPasteByPoint() {
      if (!clipboard) {
        setMode('normal', 'Mode: clipboard is empty. Copy text first.');
        return;
      }
      setMode('pasteByPoint', 'Mode: click paste point. The object will be placed relative to the copied reference point.');
    }

    function clearGeneratedCopies() {
      document.querySelectorAll('[data-generated-copy="true"]').forEach((item) => item.remove());
      document.querySelectorAll('[data-role="reference-copy-markers"]').forEach((item) => item.remove());
      setMode('normal', 'Mode: generated copies and reference markers cleared.');
    }

    function cancelMode() {
      setMode('normal', 'Mode: select text or drag labels. Ctrl+C copies from center, Ctrl+V starts paste-by-point, Esc cancels.');
    }

    function pasteClipboardAt(svg, pastePoint) {
      if (!clipboard || clipboard.kind !== 'text') return;

      generatedCopyCounter += 1;

      const offsetX = clipboard.center.x - clipboard.basePoint.x;
      const offsetY = clipboard.center.y - clipboard.basePoint.y;
      const nextCenter = {
        x: pastePoint.x + offsetX,
        y: pastePoint.y + offsetY,
      };
      const dx = nextCenter.x - clipboard.center.x;
      const dy = nextCenter.y - clipboard.center.y;
      const nextAnchor = {
        x: clipboard.anchor.x + dx,
        y: clipboard.anchor.y + dy,
      };

      const clone = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      clone.textContent = clipboard.text;
      clone.setAttribute('x', String(Math.round(nextAnchor.x * 10) / 10));
      clone.setAttribute('y', String(Math.round(nextAnchor.y * 10) / 10));
      clone.setAttribute('font-family', clipboard.attributes.fontFamily);
      clone.setAttribute('font-size', clipboard.attributes.fontSize);
      clone.setAttribute('text-anchor', clipboard.attributes.textAnchor);
      clone.setAttribute('dominant-baseline', clipboard.attributes.dominantBaseline);
      clone.setAttribute('fill', clipboard.attributes.fill);
      clone.setAttribute('data-role', 'free-text-box');
      clone.setAttribute('data-copyable-text', 'true');
      clone.setAttribute('data-generated-copy', 'true');
      clone.setAttribute('data-copy-id', `copy_${generatedCopyCounter}`);
      clone.setAttribute('data-source-role', clipboard.role);
      clone.setAttribute('data-source-bay-slot-id', clipboard.sourceBaySlotId);
      clone.setAttribute('data-rotation', String(clipboard.rotation));
      clone.setAttribute('transform', `rotate(${clipboard.rotation} ${nextAnchor.x} ${nextAnchor.y})`);

      const drawingLayer = svg.querySelector('.drawing-layer') || svg;
      drawingLayer.appendChild(clone);

      drawReferenceMarker(svg, pastePoint, 'paste');
      selectTarget(clone);
      setMode('pasteByPoint', 'Mode: pasted. Click another paste point to repeat, or Esc/Cancel to stop.');
    }
    ...
    document.getElementById('copyCenterButton').addEventListener('click', copySelectedFromCenter);
    document.getElementById('copyByReferenceButton').addEventListener('click', beginCopyByReferencePoint);
    document.getElementById('pasteByPointButton').addEventListener('click', beginPasteByPoint);
    document.getElementById('cancelModeButton').addEventListener('click', cancelMode);
    document.getElementById('clearCopiesButton').addEventListener('click', clearGeneratedCopies);
    ...
    document.addEventListener('pointerdown', (event) => {
      const svgFromTarget = event.target.closest('svg.review-svg');
      if (!svgFromTarget) return;

      const svgPoint = clientToSvgPoint(svgFromTarget, event);
      if (!svgPoint) return;

      if (currentMode === 'pickBase') {
        event.preventDefault();
        if (buildClipboardFromSelected(svgPoint)) {
          setMode('pasteByPoint', 'Mode: base point saved. Click paste point(s). Esc cancels.');
        }
        return;
      }

      if (currentMode === 'pasteByPoint') {
        event.preventDefault();
        pasteClipboardAt(svgFromTarget, svgPoint);
        return;
      }

      const target = event.target.closest('[data-role="bus-label"], [data-role="bay-label"], [data-role="free-text-box"]');
      if (!target) return;
