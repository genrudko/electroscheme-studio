...
await rm(input,{recursive:true,force:true}); await mkdir(input,{recursive:true});
...
const out=path.join(root,"evidence","generated","electron-package"); await rm(out,{recursive:true,force:true});
const result=await packager({dir:input,name:"ElectroSchemeSpikeElectron",electronVersion:"43.3.0",platform:process.platform,arch:"x64",out,overwrite:true,asar:{unpackDir:"{tools,shared/fixtures}"}});
